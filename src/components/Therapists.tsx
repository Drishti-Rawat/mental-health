...
export default function Therapists() {
  ...
  return (
    <>
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 mb-8 md:mb-12 mt-16">
        <div className="flex flex-col items-center md:items-start text-center md:text-left w-full md:w-auto">
          <span className="inline-block bg-secondary/10 text-foreground font-semibold text-[10px] md:text-xs tracking-wider uppercase rounded-full px-3 py-1.5 md:px-4 md:py-1.5 mb-3 md:mb-4">
            Meet Our Expert
          </span>
          <h2 className="font-serif text-3xl md:text-5xl font-bold text-secondary mb-3 md:mb-4">
            Our Compassionate Therapists
          </h2>
          <p className="text-foreground/70 text-sm md:text-lg max-w-xl">
            Highly qualified professionals dedicated to your well-being.
          </p>
        </div>
        <button className="hidden md:block shrink-0 px-6 py-2.5 rounded-full border-2 border-secondary text-secondary font-semibold hover:bg-tertiary transition-colors cursor-pointer">
          View All Therapists
        </button>
      </div>
        ...
        >
          <div className="flex -ml-4 py-2">
            {therapistsData.map((therapist, index) => (
              <div key={index} className="flex-[0_0_85%] sm:flex-[0_0_50%] lg:flex-[0_0_25%] xl:flex-[0_0_20%] min-w-0 pl-4">
                <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden flex flex-col h-full group transition-transform duration-300 hover:-translate-y-1 hover:shadow-md">
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    <h3 className="font-bold text-base md:text-lg text-foreground mb-1">{therapist.name}</h3>
                    ...
                    <button className="w-full bg-secondary text-white font-semibold py-2.5 rounded-xl text-sm hover:bg-secondary/90 transition-colors shadow-sm mt-auto">
                      Book a Session
                    </button>
                  </div>
                </div>
